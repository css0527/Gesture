...
/* 在文件头部，#includes 之后添加 */
#ifndef FORCE_PM
#define FORCE_PM    (1 << 17)
#endif

#ifndef LINK_IE_RX_SET_FC
#define LINK_IE_RX_SET_FC    (1 << 21)
#endif

#ifndef LINK_IF_RX_SET_FC
#define LINK_IF_RX_SET_FC    (1 << 21)
#endif
...
void USBSS_Device_Init( FunctionalState sta )                                                     
{
    if( sta )
    {
        ...
        /* 在 USBSS_Device_Init 函数中，约74行处 */
        if( Chip >= 3 )
        {
    #ifdef LINK_IE_RX_SET_FC
            USBSSD->LINK_INT_CTRL |= LINK_IE_RX_SET_FC;
    #else
            /* 使用替代定义或忽略 */
            USBSSD->LINK_INT_CTRL |= (1 << 21);  /* LINK_IE_RX_SET_FC = 1<<21 */
    #endif
        }
